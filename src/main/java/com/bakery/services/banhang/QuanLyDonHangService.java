...
public class QuanLyDonHangService {
    ...
    public QuanLyDonHangService(DonHangDAO donHangDAO) {
    ...
    public int taoDonHang(YeuCauTaoDonHangDTO request) throws Exception {
        ...
        try {
            int maDonMoi = donHangDAO.taoDonHang(donDatHang, dsCtDonHang, dsCtTuyChinh);
            boolean tonTai = donHangDAO.tonTaiDonHang(maDonMoi);
            if (!tonTai) {
    ...
    public DonDatHangDTO chuyenTrangThaiDon(int maDon, int maTrangThaiMoi, int maNvCapNhat,
        ...
        String trangThaiHienTai = com.bakery.utils.StringUtil.chuanHoa(tenTrangThaiHienTai);
        String trangThaiMoi = com.bakery.utils.StringUtil.chuanHoa(tenTrangThaiMoi);
        ...
        try {
            donHangDAO.chuyenTrangThaiDon(maDon, maTrangThaiMoi, maNvCapNhat, hinhThucNhan);

            // Nếu chuyển sang HOÀN_THÀNH → trả về đơn hàng để ThanhToanService tạo hóa đơn
            if ("HOAN_THANH".equals(trangThaiMoi)) {
                return donHangDAO.layTomTatDonHang(maDon);
            }
            return null;
        } catch (SQLException e) {
    ...
    public void huyDonVaHoanCoc(int maDon, String lyDoHuy, int maNvCapNhat,
            String tenTrangThaiHienTai, double refundAmount, int maCa) throws Exception {
        if (maDon <= 0)
        ...
        try {
            donHangDAO.huyDonVaHoanKho(maDon, lyDoHuy.trim(), maNvCapNhat, refundAmount, maCa);
        } catch (SQLException e) {
    ...
    public DonDatHangDTO layTomTatDonHang(int maDon) throws Exception {
        ...
        try {
            DonDatHangDTO tomTat = donHangDAO.layTomTatDonHang(maDon);
            if (tomTat == null)
    ...
    public List<CTDonHangDTO> layChiTietDonHang(int maDon) throws Exception {
        ...
        try {
            return donHangDAO.layChiTietDonHang(maDon);
        } catch (SQLException e) {
    ...
    public List<CTDonTuyChinhDTO> layChiTietTuyChinh(int maDon) throws Exception {
        if (maDon <= 0) throw new IllegalArgumentException("Mã đơn không hợp lệ.");
        try {
            return donHangDAO.layChiTietTuyChinh(maDon);
        } catch (SQLException e) {
    ...
    public List<TrangThaiDonDTO> layDanhSachTrangThaiDon() throws Exception {
        try {
            return donHangDAO.layDanhSachTrangThaiDon();
        } catch (SQLException e) {
    ...
    private int layMaTrangThaiDaCoc() throws Exception {
        ...
        for (TrangThaiDonDTO tt : dsTrangThai) {
            if ("DA_COC".equals(com.bakery.utils.StringUtil.chuanHoa(tt.getTenTrangThai())))
    ...
    private int layMaTrangThaiMoiDat() throws Exception {
        List<TrangThaiDonDTO> dsTrangThai = layDanhSachTrangThaiDon();
        for (TrangThaiDonDTO tt : dsTrangThai) {
            String normalized = com.bakery.utils.StringUtil.chuanHoa(tt.getTenTrangThai());
    ...
    private boolean kiemTraTrangThaiCamHuy(String tenTrangThaiHienTai) {
        String current = com.bakery.utils.StringUtil.chuanHoa(tenTrangThaiHienTai);
