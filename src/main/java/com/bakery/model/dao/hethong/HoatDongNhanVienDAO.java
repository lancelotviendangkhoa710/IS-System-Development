...
public class HoatDongNhanVienDAO extends BaseDAO {
    ...
    /**
     * Lấy danh sách hoạt động nhân viên có lọc theo nhóm, từ khóa và khoảng thời
     * gian.
     *
     * @param nhom    Nhóm module ('DON_HANG' | 'KHACH_HANG' | 'KHO' | 'SAN_PHAM' |
     *                null = tất cả)
    ...
    public List<HoatDongNhanVienDTO> layDanhSach(String nhom, String tuKhoa,
        ...
        StringBuilder sql = new StringBuilder(
                "SELECT MAHOATDONG, MANV, TENNHANVIEN, VAITRO, NHOM, HANHDONG, ENTITY_ID, THOIGIAN " +
                        "FROM VW_HoatDongNhanVien WHERE 1=1");
        if (nhom != null && !nhom.isBlank())
            sql.append(" AND NHOM = ?");
        if (tuKhoa != null && !tuKhoa.isBlank())
            sql.append(" AND (UPPER(TENNHANVIEN) LIKE ? OR UPPER(HANHDONG) LIKE ?)");
        if (tuNgay != null)
        ...
        try (Connection con = moKetNoi();
            ...
            if (denNgay != null)
                ps.setDate(idx++, Date.valueOf(denNgay.plusDays(1)));
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    dto.setChucVu(rs.getString("VAITRO"));
                    ...
                    if (ts != null)
                        dto.setThoiGian(ts.toLocalDateTime());
                    ds.add(dto);
                }
            }
        } catch (Exception e) {
