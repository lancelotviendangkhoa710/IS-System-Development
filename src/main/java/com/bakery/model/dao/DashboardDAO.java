...
public class DashboardDAO {
    ...
    /**
     * Lấy 4 chỉ số KPI cho Dashboard trong một lần kết nối DB.
     * TODO: Kiểm tra lại tên bảng DONHANG, NGUYENLIEU và các cột TRANGTHAI với DBA.
     */
    public DashboardKPIDTO layKPI() throws SQLException {
        ...
        try (Connection conn = DBConnect.getConnection()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlHoaDon);
                 ResultSet rs = ps.executeQuery()) {
                if (rs.next()) soHoaDon = rs.getInt(1);
            }

            // 3. Đơn đang xử lý (chưa hoàn thành, chưa hủy)
            // TODO: Xác nhận tên bảng và giá trị TRANGTHAI với DBA
            String sqlDangXuLy =
                "SELECT COUNT(*) " +
                "FROM   DONHANG " +
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlDangXuLy);
                 ResultSet rs = ps.executeQuery()) {
                if (rs.next()) donDangXuLy = rs.getInt(1);
            }

            // 4. Cảnh báo tồn kho nguyên liệu dưới mức an toàn
            // TODO: Xác nhận tên bảng nguyên liệu và cột mức tối thiểu với DBA
            String sqlTonKho =
                "SELECT COUNT(*) " +
                "FROM   NGUYENLIEU " +
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlTonKho);
                 ResultSet rs = ps.executeQuery()) {
                if (rs.next()) canhBaoTonKho = rs.getInt(1);
            }
        }

        return new DashboardKPIDTO(doanhThu, soHoaDon, donDangXuLy, canhBaoTonKho);
    }

    /**
     * Top 5 sản phẩm bán chạy nhất trong 30 ngày gần nhất.
     * TODO: Xác nhận tên bảng CTDONHANG, DONHANG và các cột với DBA.
     */
