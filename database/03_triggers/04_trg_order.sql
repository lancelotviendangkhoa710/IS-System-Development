...
BEGIN
    ...
    BEGIN
        SELECT NVL(EXTRACT(DAY FROM CAST(THOIGIANCHUANBI AS TIMESTAMP)), 0) INTO V_THOIGIANCHUANBI FROM SANPHAM WHERE MASP = V_MASP;
    EXCEPTION
...
BEGIN
    IF INSERTING THEN
        V_CHENHLECH := :NEW.SOLUONG;
        V_MADON := :NEW.MADON;
        V_THOIGIANCHUANBI := NVL(EXTRACT(DAY FROM CAST(:NEW.THOIGIANCHUANBI AS TIMESTAMP)), 0);
    ELSIF UPDATING THEN
        V_CHENHLECH := :NEW.SOLUONG - :OLD.SOLUONG;
        V_MADON := :NEW.MADON;
        V_THOIGIANCHUANBI := NVL(EXTRACT(DAY FROM CAST(:NEW.THOIGIANCHUANBI AS TIMESTAMP)), 0);
    ELSIF DELETING THEN
        V_CHENHLECH := -(:OLD.SOLUONG);
        V_MADON := :OLD.MADON;
        V_THOIGIANCHUANBI := NVL(EXTRACT(DAY FROM CAST(:OLD.THOIGIANCHUANBI AS TIMESTAMP)), 0);
    END IF;
