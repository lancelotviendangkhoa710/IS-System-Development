...
FOR EACH ROW
DECLARE
    V_NGAYNHAP PHIEUNHAPKHO.NGAYNHAP%type;
BEGIN
    -- Chống gian lận hạn sử dụng
    IF UPDATING THEN
        IF :NEW.HANSUDUNG > :OLD.HANSUDUNG THEN
            RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_GIAN_LAN_HSD,
    ...
    IF TRUNC(:NEW.HANSUDUNG) - 10 <= TRUNC(V_NGAYNHAP) THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HSD_KHONG_HOPLE,
            'LOI NGHIEP VU: Han su dung cua lo nay (' || TO_CHAR(:NEW.HANSUDUNG, 'DD/MM/YYYY') ||
            ') va ngay nhap kho (' || TO_CHAR(V_NGAYNHAP, 'DD/MM/YYYY') || ') da qua han hoac gan het han!.');
    END IF;

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KHONG_CO_PHIEUNHAP, 'Loi: Khong tim thay phieu nhap kho.');
END;
/