-- Procedure thêm hạng thành viên
CREATE OR REPLACE PROCEDURE PROC_THEM_HANGTHANHVIEN (
    P_TENHANG IN HANGTHANHVIEN.TENHANG%type,
    P_DIEMTOITHIEU IN HANGTHANHVIEN.DIEMTOITHIEU%type,
    P_PHANTRAMGIAMGIA IN HANGTHANHVIEN.PHANTRAMGIAMGIA%type,
    P_MAHANG_OUT OUT HANGTHANHVIEN.TENHANG%type
)
IS
BEGIN
    INSERT INTO HANGTHANHVIEN (TENHANG, DIEMTOITHIEU, PHANTRAMGIAMGIA, THOIDIEMXOA, MANX)
    VALUES (P_TENHANG, NVL(P_DIEMTOITHIEU, 0), NVL(P_PHANTRAMGIAMGIA, 0), NULL, NULL)
    RETURNING MAHANG INTO P_MAHANG_OUT;
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HANGTV_THEM_HE_THONG, 'Loi he thong khi them Hang thanh vien: ' || SQLERRM);
END;
/

-- Procedure sửa hạng thành viên
CREATE OR REPLACE PROCEDURE PROC_SUA_HANGTHANHVIEN (
    P_MAHANG IN HANGTHANHVIEN.MAHANG%type,
    P_TENHANG IN HANGTHANHVIEN.TENHANG%type,
    P_DIEMTOITHIEU IN HANGTHANHVIEN.DIEMTOITHIEU%type,
    P_PHANTRAMGIAMGIA IN HANGTHANHVIEN.PHANTRAMGIAMGIA%type
)
IS
BEGIN
    UPDATE HANGTHANHVIEN
    SET TENHANG = NVL(P_TENHANG, TENHANG),
        DIEMTOITHIEU = NVL(P_DIEMTOITHIEU, DIEMTOITHIEU),
        PHANTRAMGIAMGIA = NVL(P_PHANTRAMGIAMGIA, PHANTRAMGIAMGIA)
    WHERE MAHANG = P_MAHANG;

    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HANGTV_KHONG_TON_TAI_CN, 'Loi: Khong tim thay Hang thanh vien de cap nhat.');
    END IF;

    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        IF SQLCODE = PKG_ERROR_CODES.ERR_HANGTV_KHONG_TON_TAI_CN THEN RAISE; END IF;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HANGTV_CAPNHAT_HE_THONG, 'Loi he thong khi cap nhat Hang thanh vien: ' || SQLERRM);
END;
/

-- Procedure xóa hạng thành viên
CREATE OR REPLACE PROCEDURE PROC_XOA_HANGTHANHVIEN (
    P_MAHANG IN HANGTHANHVIEN.MAHANG%type,
    P_MANV_XOA IN NHANVIEN.MANV%type
)
IS
BEGIN
    UPDATE HANGTHANHVIEN
    SET THOIDIEMXOA = SYSDATE,
        MANX = P_MANV_XOA
    WHERE MAHANG = P_MAHANG;

    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HANGTV_KHONG_TON_TAI_XOA, 'Loi: Khong tim thay Hang thanh vien de xoa.');
    END IF;

    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        IF SQLCODE = PKG_ERROR_CODES.ERR_HANGTV_KHONG_TON_TAI_XOA THEN RAISE; END IF;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_HANGTV_XOA_HE_THONG, 'Loi he thong khi xoa Hang thanh vien: ' || SQLERRM);
END;
/

-- Procedure thêm khách hàng
CREATE OR REPLACE PROCEDURE PROC_THEM_KHACHHANG (
    P_HOTEN IN NVARCHAR2,
    P_SDT IN VARCHAR2,
    P_DIACHI IN NVARCHAR2,
    P_DIEMTICHLUY IN NUMBER DEFAULT 0,
    P_MAHANG IN NUMBER DEFAULT NULL,
    P_MAKH_OUT OUT NUMBER,
    P_MANV_LAP IN NUMBER DEFAULT NULL
)
IS
BEGIN
    INSERT INTO KHACHHANG (HOTEN, SDT, DIACHI, NGAYDANGKY, DIEMTICHLUY, MAHANG, THOIDIEMXOA, MANX)
    VALUES (P_HOTEN, P_SDT, P_DIACHI, SYSDATE, P_DIEMTICHLUY, P_MAHANG, NULL, NULL)
    RETURNING MAKH INTO P_MAKH_OUT;

    -- Ghi log hoạt động nếu có MANV
    IF P_MANV_LAP IS NOT NULL THEN
        INSERT INTO HOATDONGNHANVIEN (MANV, NHOM, HANHDONG, ENTITY_ID)
        VALUES (P_MANV_LAP, 'KHACH_HANG', 'Them KH: ' || P_HOTEN || ' (' || P_SDT || ')', P_MAKH_OUT);
    END IF;

    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KH_THEM_HE_THONG, 'Loi he thong khi them Khach hang: ' || SQLERRM);
END;
/

-- Procedure sửa khách hàng
CREATE OR REPLACE PROCEDURE PROC_SUA_KHACHHANG (
    P_MAKH IN KHACHHANG.MAKH%type,
    P_HOTEN IN KHACHHANG.HOTEN%type,
    P_SDT IN KHACHHANG.SDT%type,
    P_DIACHI IN KHACHHANG.DIACHI%type,
    P_DIEMTICHLUY IN KHACHHANG.DIEMTICHLUY%type,
    P_MAHANG IN KHACHHANG.MAHANG%type,
    P_MANV_CAP IN NUMBER DEFAULT NULL
)
IS
BEGIN
    UPDATE KHACHHANG
    SET HOTEN = NVL(P_HOTEN, HOTEN),
        SDT = NVL(P_SDT, SDT),
        DIACHI = NVL(P_DIACHI, DIACHI),
        DIEMTICHLUY = NVL(P_DIEMTICHLUY, DIEMTICHLUY),
        MAHANG = NVL(P_MAHANG, MAHANG)
    WHERE MAKH = P_MAKH;

    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KH_KHONG_TON_TAI_CN, 'Loi: Khong tim thay Khach hang de cap nhat.');
    END IF;

    IF P_MANV_CAP IS NOT NULL THEN
        INSERT INTO HOATDONGNHANVIEN (MANV, NHOM, HANHDONG, ENTITY_ID)
        VALUES (P_MANV_CAP, 'KHACH_HANG', 'Cap nhat KH #' || P_MAKH, P_MAKH);
    END IF;

    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        IF SQLCODE = PKG_ERROR_CODES.ERR_KH_KHONG_TON_TAI_CN THEN RAISE; END IF;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KH_CAPNHAT_HE_THONG, 'Loi he thong khi cap nhat Khach hang: ' || SQLERRM);
END;
/

-- Procedure xóa khách hàng
CREATE OR REPLACE PROCEDURE PROC_XOA_KHACHHANG (
    P_MAKH IN KHACHHANG.MAKH%type,
    P_MANV_XOA IN NHANVIEN.MANV%type
)
IS
BEGIN
    UPDATE KHACHHANG
    SET THOIDIEMXOA = SYSDATE,
        MANX = P_MANV_XOA
    WHERE MAKH = P_MAKH;

    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KH_KHONG_TON_TAI_XOA, 'Loi: Khong tim thay Khach hang de xoa.');
    END IF;

    INSERT INTO HOATDONGNHANVIEN (MANV, NHOM, HANHDONG, ENTITY_ID)
    VALUES (P_MANV_XOA, 'KHACH_HANG', 'Xoa KH #' || P_MAKH, P_MAKH);

    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        IF SQLCODE = PKG_ERROR_CODES.ERR_KH_KHONG_TON_TAI_XOA THEN RAISE; END IF;
        RAISE_APPLICATION_ERROR(PKG_ERROR_CODES.ERR_KH_XOA_HE_THONG, 'Loi he thong khi xoa Khach hang: ' || SQLERRM);
END;
/
