...
-- FOR UPDATE NOWAIT: là khóa dòng ngay, không chờ nếu bị session khác giữ
...
-- Kiểm tra tồn kho toàn bộ nguyên liệu trước khi xuất bất kỳ lô nào
...
-- Duyệt lại từng nguyên liệu để thực hiện xuất kho FEFO
-- Lần này mở cursor FEFO có FOR UPDATE NOWAIT để khóa dòng
    FOR V_ROWCT IN C_CONGTHUC LOOP
...
-- Bao gồm: INSERT PHIEUXUATKHO + tất cả UPDATE CTPHIEUNHAP + tất cả INSERT CTPHIEUXUAT_NL
-- của mọi nguyên liệu
    COMMIT;
